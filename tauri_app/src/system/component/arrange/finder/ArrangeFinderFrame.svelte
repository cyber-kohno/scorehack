...
import type ArrangeLibrary from "../../../../domain/arrange/arrange-library";
  ...
  import { setArrangeFinderFrameRef } from "../../../../state/session-state/arrange-ref-store";
  import { outlineArrangeStore } from "../../../../state/session-state/outline-arrange-store";

  let ref: HTMLElement | null = null;
  onMount(() => {
    if (ref != null) {
      setArrangeFinderFrameRef(ref);
    ...
    return () => {
      setArrangeFinderFrameRef(undefined);
    };
  });

  $: finder = (() => {
    const finder = $outlineArrangeStore?.finder;
    if (finder == null) throw new Error("finder is null");
