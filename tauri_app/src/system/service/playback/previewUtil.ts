...
namespace PreviewUtil {
  ...
  export const useUpdater = (storeUtil: StoreUtil) => {
    ...
    const startTest = (option: Option) => {
      ...
      const { getCurrScoreTrack } = useMelodyUpdater(lastStore);
      
      const reducerArrange = ArrangeUtil.useReducer(control, data);
    ...
    const stopTest = () => {
      const { syncCursorFromElementSeq } = useMelodyUpdater(lastStore);

      const preview = lastStore.preview;
      if (preview.timerKeys == null)
        throw new Error("cache.timerKeys must not be null.");
