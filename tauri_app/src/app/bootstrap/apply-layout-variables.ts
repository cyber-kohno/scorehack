...
export const applyStaticLayoutVariables = () => {
  const PL = Layout.pitch;
  const OL = Layout.outline;
  const EL = Layout.element;
  const AP = Layout.arrange.piano;

  const voicingTableWidth = AP.VOICING_ITEM_WIDTH * AP.VOICING_STRUCT_MAX;
  ...
  setCustomProps([
    ...
    { key: "--ap-voicing-table-width", value: `${voicingTableWidth}px` },
    {
      key: "--ap-voicing-frame-width",
      value: `${AP.VOICING_STRUCT_WIDTH + voicingTableWidth}px`,
    },
    {
      key: "--ap-voicing-frame-height",
      value: `${AP.VOICING_ITEM_HEIGHT * AP.VOICING_STRUCT_MAX}px`,
    },
    { key: "--ap-backing-len-height", value: `${AP.BACKING_LEN_HEIGHT}px` },
    { key: "--ap-backing-measure-height", value: `${AP.BACKING_MEASURE_HEIGHT}px` },
    {
      key: "--ap-backing-header-height",
      value: `${AP.BACKING_LEN_HEIGHT + AP.BACKING_MEASURE_HEIGHT}px`,
    },
    ...
    {
      key: "--ap-backing-table-height",
      value: `${(AP.BACKING_RECORD_HEIGHT + 1) * AP.BACKING_RECORD_MAX + 1}px`,
    },
...
export const applyDynamicLayoutVariables = (lastStore: StoreProps) => {
  const beatSum = lastStore.cache.chordCaches.reduce(
    (total, cur) => total + cur.viewPosWidth,
    0,
  );
